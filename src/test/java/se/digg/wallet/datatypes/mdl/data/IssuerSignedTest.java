...
class IssuerSignedTest {
  ...
  void testSubject() throws Exception {
      ...
      .nameSpace(
        ...
        List.of(
          ...
          IssuerSignedItem.builder()
            ...
            .random(
              Hex.decode(
                "14696485153f5a99856679e07a053881a18800175c34ef78bdac528cff2032b0"
              )
            )
    ...
    byte[] issuerSignedCbor = CBORUtils.CBOR_MAPPER.writeValueAsBytes(
      issuerSigned
    );
    IssuerSigned deserializedIssuerSigned = CBORUtils.CBOR_MAPPER.readValue(
      issuerSignedCbor,
      IssuerSigned.class
    );
  ...
  void hashValues() throws Exception {
    ...
    logIssuerSigned(issuerSigned);
    List<IssuerSignedItem> issuerSignedItems = issuerSigned
      .getNameSpaces()
      .get("org.iso.18013.5.1");

    List<byte[]> expectedHashValues = Arrays.asList(
      new byte[] {},
      Hex.decode(
      ...
      Hex.decode(
        "7234f61d8824a319f058ffbd8ea3bfebe0fe77af47f545b6c63a435ab92796f0"
      )
    );
    for (IssuerSignedItem item : issuerSignedItems) {
      byte[] bytes = sha256Hash(item.toBeHashedBytes());
      Assertions.assertArrayEquals(
        expectedHashValues.get(item.getDigestID()),
        bytes
      );
    }
  }
  ...
  void testSignedIssuerSigned() throws Exception {
    ...
    boolean valid = parsedSignatureObject.validate(
      new COSEKey(signingCert.getPublicKey(), null)
    );
  ...
  void testIsoNameSpaceExample() throws Exception {
    ...
    byte[] mdlExample = Base64.getUrlDecoder()
      .decode(
