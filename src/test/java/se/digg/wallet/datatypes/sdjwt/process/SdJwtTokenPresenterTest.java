...
class SdJwtTokenPresenterTest {
  ...
  void testCases() throws Exception {
    ...
    byte[] ecTokenRsaWallet = SdJwtTokenValidatorTest.getToken(
      tokenIssuer,
      TestCredentials.p256_issuerCredential,
      false,
      rsaWalletPublic
    );
    byte[] ecTokenLegacy = SdJwtTokenValidatorTest.getToken(
      tokenIssuer,
      TestCredentials.p256_issuerCredential,
      true,
      walletPublic
  ...
  void performTestCase(
    String description,
    SdJwtTokenPresenter presenter,
    SdJwtPresentationInput input,
    ...
    if (expectedException != null) {
      ...
      SdJwtTokenIssuerTest.logToken(
        new String(presentedToken),
        input.getAlgorithm().getDigestAlgorithm().getJdkName()
      );
