...
public enum TokenSigningAlgorithm {
  ...
  public static TokenSigningAlgorithm fromCOSEKey(COSEKey signingKey)
    ...
    return Arrays.stream(values())
        .filter(
            tokenSigningAlgorithm -> signingKey
                .HasAlgorithmID(tokenSigningAlgorithm.getAlgorithmID()))
        .findFirst()
        .orElseThrow(
   ...
   * @throws NoSuchAlgorithmException if no supported TokenSigningAlgorithm matches the specified
   *         JWSAlgorithm
   */
  public static TokenSigningAlgorithm fromJWSAlgorithm(
      JWSAlgorithm jwsAlgorithm) throws NoSuchAlgorithmException {
    return Arrays.stream(values())
        .filter(
            signingAlgorithm -> signingAlgorithm.getJwsAlgorithm().equals(jwsAlgorithm))
        .findFirst()
        .orElseThrow(
            () -> new NoSuchAlgorithmException(
  ...
  /**
   * Creates and returns a {@link JWSSigner} based on the provided private key and the current
   * signing algorithm.
   *
   * @param privateKey the private key used for creating the signer. It must match the algorithm
   *        type expected by the signer. For EC algorithms, the key must be of type
   *        {@link ECPrivateKey}.
   * @return an instance of {@link JWSSigner} that corresponds to the current signing algorithm.
   * @throws JOSEException if an error occurs during the creation of the signer, such as invalid key
   *         type or unsupported algorithm.
   */
   ...
   *
   * This method returns a specific verifier implementation depending on the token signing algorithm
   * (e.g., ECDSA or RSA_PSS). It requires a public key and throws a JOSEException if the verifier
