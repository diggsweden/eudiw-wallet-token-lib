...
/**
 * MdlTokenPresenter is an implementation of the {@code TokenPresenter} interface specialized for
 * processing mDL (mobile Driver's License) tokens. This class is responsible for verifying the
 * provided mDL token, applying selective disclosures, and producing a cryptographically signed
 * response suitable for presentation.
 *
...
public class MdlTokenPresenter implements TokenPresenter<MdlPresentationInput> {
  ...
  /**
   * Filters and returns a subset of the provided namespaces based on the disclosed element
   * identifiers.
   *
   * @param nameSpaces a map where keys represent namespace names, and values are lists of
   *        IssuerSignedItem instances representing signed attributes in those namespaces
   * @param disclosures a map where keys represent namespace names, and values are lists of
   *        attribute name identifiers that should be disclosed
   * @return a map containing only the entries from the input nameSpaces that match the disclosed
   *         namespaces and element identifiers provided in disclosures
   */
