...
/**
 * Validator for validating @{link {@link IssuerSigned} tokens. The IssuerSigned element is the
 * Issuer provided part of a EUDI wallet mDL token. This is then completed in the Wallet to produce
 * a complete verifiable credential presentation in the form of a mDoc
 *
 * <p>
 * The user attributes in the IssuerSigned object is not masked, but individually signed by the
 * signature. Selective disclosure is achieved by deleting the attributes that should not be
 * revealed from this token. This then does not break the signature.
 * </p>
 *
 * <p>
 ...
 *
 * <ul>
 * <li>Key validation if a list of trusted keys is provided. Otherwise testing is a key is provided
 * and validates the signature</li>
 * <li>Checking issue date and validity</li>
 * <li>Validating the signature</li>
 * <li>Validating the signed hash of all present attributes. (This works also if some attributes are
 * deleted)</li>
 * </ul>
 */
public class MdlIssuerSignedValidator implements TokenValidator {
   ...
   *
   * @param timeSkew the duration representing the time difference allowed between the client and
   *        server in verifying tokens.
   */
  ...
  /**
   * Initializes a new instance of the MdlIssuerSignedValidator class with a default time skew of 30
   * seconds.
   */
   ...
   * @throws TokenValidationException if the MobileSecurityObject does not contain validity
   *         information, the signing time is not declared, the valid from time is not declared, the
   *         expiration time is not declared, the token declares signing time in the future, the
   *         token is not yet valid, or the token has expired
   */
  ...
  /**
   * Validates the attributes of a MobileSecurityObject against a map of namespaces and signed
   * items.
   *
   * @param nameSpaces a map containing namespace strings as keys and lists of IssuerSignedItems as
   *        values
  ...
  private void validateAttributes(
      ...
      byte[] token) throws TokenValidationException, NoSuchAlgorithmException {
    if (nameSpaces == null) {
      ...
      for (IssuerSignedItem item : items) {
        ...
        MessageDigest digest = MessageDigest.getInstance(
            TokenDigestAlgorithm.fromMdlName(
                mso.getDigestAlgorithm()).getJdkName());
  ...
  private Map<String, Map<Integer, byte[]>> parseTokenIssuerSignedItems(
      byte[] token) throws TokenValidationException {
    try {
  ...
  /**
   * Retrieves the hash value corresponding to a given name space and digest ID from the provided
   * value digests map.
   *
   ...
   * @return the byte array representing the hash value, or null if the value digests map is null,
   *         the nameSpace is not found, or the digest ID is not found within the specified
   *         nameSpace
   */
  ...
  /**
   * Retrieves the Key Identifier (kid) from the provided Sign1COSEObject, preferring the key from
   * protected attributes if present or else from unprotected attributes.
   *
   * <p>
   * Note that the COSE standard states that KID can be stored in unprotected attributes but does
   * not forbid storing it in protected attributes. The reason to look in protected attributes
   * first, is because that information is signed and hence more trustworthy.
   * </p>
   *
  ...
  /**
   * Retrieves a list of X.509 certificates from a CBORObject representing an X.509 certificate
   * chain.
   *
   * @param x5chain the CBORObject representing the X.509 certificate chain or null if no chain was
   *        provided
