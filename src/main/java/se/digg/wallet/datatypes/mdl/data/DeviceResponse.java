...
/**
 * The DeviceResponse class represents a mDL presentation token as it is returned as response to a
 * presentation request with information such as document type, status, version, and associated
 * cryptographic data. It provides functionality for serialization and deserialization using CBOR
 * (Concise Binary Object Representation). The object can be converted into and parsed from a
 * CBOR-encoded byte array for data transmission.
 * <p>
 * This class includes the following key components: - Metadata about the response, such as document
 * type, version, and status. - Cryptographic elements, including issuer-signed data, device
 * signature, and optional device MAC key. - A serializer implementation for converting the object
 * to CBOR format.
 */
@Data
@AllArgsConstructor
...
public class DeviceResponse {
  ...
  /**
   * Constructor for the DeviceResponse class. Initializes a new instance of the DeviceResponse with
   * the specified parameters. including signature device authentication
   *
  ...
  /**
   * Constructor for the DeviceResponse class. Initializes a new instance of the DeviceResponse with
   * the specified parameters including MAC device authentication.
   *
  ...
  /**
   * The object providing the name spaces data for the device signature. By default, this is an
   * empty map.
   */
   ...
   * Exception Handling:
   * <ul>
   * <li>Throws {@link IOException} for errors during the serialization process or CBOR
   * encoding.</li>
