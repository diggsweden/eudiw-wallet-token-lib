 ...
 * <p>
 * Features include: - Representation of key and validity information related to the security of
 * mobile devices. - CBOR serialization and deserialization for the MSO and its nested objects. -
 ...
 * Nested structures:
 * <ul>
 * <li>{@code DeviceKeyInfo}: Contains key metadata, authorizations, and other key-related
 * details.</li>
 * <li>{@code ValidityInfo}: Represents validity timestamps like signed time, valid start, and
 * expiration.</li>
 * <li>{@code KeyAuthorizations}: Includes key authorizations pertaining to nameSpaces and data
 * elements.</li>
 * </ul>
 *
...
public class MobileSecurityObject {
   ...
   */
  public static class MobileSecurityObjectBuilder {
  } // lombok workaround

  // https://stackoverflow.com/questions/51947791/javadoc-cannot-find-symbol-error-when-using-lomboks-builder-annotation

  /**
   * Represents information related to a device key in the MobileSecurityObject. This class includes
   * the key itself, associated authorizations, and additional metadata.
   */
  @Data
  @AllArgsConstructor
  @NoArgsConstructor
  ...
  /**
   * Represents the validity information of a digital security object. This class contains the
   * timestamps related to the signing, validity period, and expected update of the associated
   * security object.
   */
  @Data
  @AllArgsConstructor
  ...
  /**
   * The KeyAuthorizations class represents key authorization details, specifying access permissions
   * for certain namespaces and data elements.
   */
  @Data
  @AllArgsConstructor
  @NoArgsConstructor
  ...
  /**
   * Signs the Mobile Security Object (MSO) using the specified certificate chain, key, and
   * algorithm ID.
   *
   * @param chain the list of {@link X509Certificate} representing the certificate chain used for
   *        signing
   ...
   * @throws CertificateEncodingException if an error occurs while encoding the certificates in the
   *         chain
   */
  @JsonIgnore
  public CBORObject sign(
      ...
      COSEKey key,
      AlgorithmID algorithmID) throws IOException, CoseException, CertificateEncodingException {
    return sign(chain, key, algorithmID, null, false);
  }

  /**
   * Signs the Mobile Security Object (MSO) using the specified certificate chain, key, algorithm
   * ID, key identifier (KID), and KID protection flag.
   *
   * @param chain the list of {@link X509Certificate} representing the certificate chain used for
   *        signing
   * @param key the {@link COSEKey} used to create the digital signature
   ...
   * @param protectedKid a boolean flag indicating whether the KID is included as a protected
   *        attribute
   * @return the signed object as a {@link CBORObject}
   * @throws IOException if an I/O error occurs during the signing process
   * @throws CoseException if an error related to the COSE signing process occurs
  ...
  public CBORObject sign(
      ...
      String kid,
      boolean protectedKid) throws IOException, CoseException, CertificateEncodingException {
  ...
  /**
   * The Serializer class is a custom implementation of the {@link JsonSerializer} specifically for
   * the {@link MobileSecurityObject} class. It is responsible for serializing an instance of
   * {@link MobileSecurityObject} into a CBOR encoded representation using the
   * {@link JsonGenerator}.
   * <p>
   * The serialization process involves encoding various fields of the {@link MobileSecurityObject},
   * such as version, digest algorithm, value digests, device key information, document type, and
   * validity information, into their respective CBOR representations.
   * <p>
   * This class ensures that the serialization adheres to the required format and structure, and
   * includes detailed handling of nested objects like {@link MobileSecurityObject.DeviceKeyInfo}
   * and {@link MobileSecurityObject.KeyAuthorizations}. It also takes care of encoding maps, lists,
   * and byte arrays in a structured and deterministic manner using CBOR utilities.
   */
