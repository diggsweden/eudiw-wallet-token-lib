...
public class Disclosure {
  ...
  /**
   * Constructs a new Disclosure instance using the provided TokenAttribute. This constructor
   * generates a random salt, extracts relevant information from the TokenAttribute, and creates a
   * base64-encoded disclosure string.
   *
   * @param tokenAttribute the TokenAttribute object containing the type (name) and value of the
   *        attribute to be disclosed. If the type is not null, its name will be included in the
   *        disclosure along with the value; otherwise, only the value will be included
   * @throws JsonProcessingException if there is an error during the JSON serialization process
   *         while creating the disclosure string
   */
   ...
   * This constructor decodes the string, parses it into components, and initializes the salt, name,
   * and value fields based on the content of the parsed disclosure string.
   *
   * @param disclosureB64Url the base64 URL-encoded string representing the disclosure. This string
   *        must contain the salt as the first value and can optionally include the name of the
   *        attribute (as the second value) and the attribute value
   *
   * @throws JsonProcessingException if there is an error during the JSON parsing process
   * @throws TokenValidationException if the disclosure format is invalid, i.e., the decoded and
   *         parsed data does not meet the expected structure
   */
