...
/**
 * A concrete implementation of the {@link TokenIssuer} interface responsible for issuing SD-JWT
 * tokens.
 * <p>
 * This class handles the generation of signed (SD-JWT) tokens with selective disclosure. The
 * implementation ensures tokens are generated in compliance with the specified signing algorithm,
 * issuer credentials, and attributes provided in the {@link SdJwtTokenInput}. It supports
    ...
    try {
      ...
      if (claimsWithDisclosure == null) {
        ...
        List<TokenAttribute> tokenAttributes = Optional.ofNullable(
            tokenInput.getAttributes()).orElse(new ArrayList<>());
        ...
        List<TokenAttribute> openAttributes = Optional.ofNullable(
            tokenInput.getOpenAttributes()).orElse(new ArrayList<>());
