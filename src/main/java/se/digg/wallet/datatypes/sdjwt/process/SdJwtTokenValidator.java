...
/**
 * The SdJwtTokenValidator class is responsible for the validation of SD-JWT tokens. It implements
 * the {@code TokenValidator} interface and provides functionality to parse, verify, and validate
 * tokens following the SD-JWT specification, including the reconstruction of selective disclosure
 * claims, time-based validations, key binding proof verification, and certificate chain validation.
 */
...
public class SdJwtTokenValidator implements TokenValidator {
   ...
   *
   * @param timeSkew The accepted time skew duration for token validation to account for clock
   *        differences.
   */
  ...
  /**
   * Default constructor for the SdJwtTokenValidator class. Initializes the validator with a default
   * time skew of 30 seconds. The time skew is used to account for clock differences during token
   * validation.
   */
  ...
  /**
   * Validates an SD-JWT token against provided trusted keys and internal rules. Performs signature
   * verification, time validation, key binding validation, and payload restoration.
   *
   * @param token The SD-JWT token as a byte array.
   * @param trustedKeys A list of optional trusted keys to validate the token against. May be null
   *        to trust all keys.
   * @return An instance of {@code SdJwtTokenValidationResult} that contains the details of the
   *         validated token and any extracted data.
   * @throws TokenValidationException If token validation fails due to invalid signature, expired
   *         token, untrusted signing certificate, or violation of key binding mechanisms.
   */
  @Override
  ...
  /**
   * Expands the claims in a map by resolving selective disclosure fields and integrating matching
   * disclosures. The method processes nested claim structures and iteratively replaces selective
   * disclosure references with their corresponding values from a list of provided disclosures.
   *
   * @param claims a map containing the original claims of the token which may include selective
   *        disclosure references that need to be resolved
   * @param allDisclosures a list of Disclosure objects representing the available disclosures from
   *        which values can be extracted
  ...
  /**
   * Finds and returns a matching {@code Disclosure} instance from a list of disclosures based on a
   * base64 URL-encoded hash value and a specified hash algorithm. If no matching disclosure is
  ...
  /**
   * Validates the key binding by verifying the wallet signature and comparing the hash. If no key
   * binding is available a false value is returned. If a key binding is available but fails
   * validation, an exception is thrown.
   *
   ...
   *
   * @param issuerSignedClaims The JWT claims set signed by the issuer containing the wallet public
   *        key information.
   * @return The wallet's {@code PublicKey} extracted and parsed from the claims.
   * @throws TokenValidationException If no wallet public key is found or if an error occurs during
   *         parsing of the wallet public key.
   */
  private PublicKey getWalletPublic(JWTClaimsSet issuerSignedClaims)
      throws TokenValidationException {
    try {
      JWK walletPublic = Optional.ofNullable(
          SdJwt.parseConfirmationKey(issuerSignedClaims.getClaim("cnf"))).orElseThrow(
   ...
   * @throws TokenValidationException If the token's issue time or expiration time is missing, or if
   *         the current time is outside the valid time window.
   */
  private void timeValidation(JWTClaimsSet jwtClaimsSet)
      throws TokenValidationException {
    Instant issueTime = Optional.ofNullable(
        jwtClaimsSet.getIssueTime().toInstant()).orElseThrow(
    ...
    Instant expirationTime = Optional.ofNullable(
        jwtClaimsSet.getExpirationTime().toInstant()).orElseThrow(
  ...
  /**
   * Retrieves the public key used to validate a token by determining whether the provided key in
   * the certificate chain matches any of the trusted keys or key IDs.
   *
   * @param chain A list of X509 certificate chain whose public key may be used for validation. If
   *        the chain is empty, no public key is provided.
   * @param kid The key identifier (key ID) associated with the token for which validation is being
   *        performed. This is used to locate a matching trusted key.
   * @param trustedKeys A list of trusted keys to validate the provided key against. If null, all
   *        keys are considered as trusted.
   * @return The public key to be used for validation if the provided key matches a trusted key or
   *         key ID.
   * @throws TokenValidationException If no matching trusted key is found and trusted keys were
   *         explicitly provided.
   */
    ...
    if (!allTrusted) {
      for (TrustedKey trustedKey : trustedKeys) {
        PublicKey trustedPublicKey = Optional.ofNullable(
            trustedKey.getPublicKey()).orElse(trustedKey.getCertificate().getPublicKey());
  ...
  /**
   * Converts a list of Base64-encoded certificates into a list of X509Certificate objects. Decodes
   * each Base64 entry, parses the certificate data, and constructs the corresponding
   * X509Certificate. If the input list is null, an empty list is returned.
   *
