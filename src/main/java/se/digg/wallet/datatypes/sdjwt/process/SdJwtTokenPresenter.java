 ...
 * <p>
 * This class is responsible for taking an SdJwt token along with its associated input parameters
 * (nonce, audience, algorithm, etc.) and producing a verifiable presentation. The verifiable
 * presentation is signed using the provided private key.
 */
public class SdJwtTokenPresenter
    implements TokenPresenter<SdJwtPresentationInput> {

  /**
   * Default constructor for the SdJwtTokenPresenter class.
   * <p>
   * This constructor initializes a new instance of the SdJwtTokenPresenter, a concrete
   * implementation of the TokenPresenter interface. The presenter is used for generating verifiable
   * presentations of SdJwt tokens with selective disclosures.
   * <p>
   * SdJwtTokenPresenter is typically used in scenarios where verifiable credentials need to be
   * presented in a secure and signed format, ensuring compliance with selective disclosure
   * mechanisms.
   */
  ...
  public byte[] presentToken(
      ...
      PrivateKey privateKey) throws TokenPresentationException {
    if (presentationInput instanceof SdJwtPresentationInput input) {
      try {
        Objects.requireNonNull(
            input.getToken(),
            "Input token must not be null");
        ...
        return protectedVerifiablePresentation.getBytes();
          ...
          TokenValidationException | JOSEException | NoSuchAlgorithmException e) {
        throw new TokenPresentationException(
            "Unable to create verifiable presentation",
            e);
      } catch (Exception e) {
        throw new TokenPresentationException(
            "Critical error while creating verifiable presentation",
            e);
      }
    } else {
      throw new TokenPresentationException(
          "PresentationInput must be of type SdJwtPresentationInput");
    }
  }
}
