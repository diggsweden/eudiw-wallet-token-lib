...
/**
 * This class represents all claims on one level (typically the base level). But any claim expressed
 * here may declare a substructure. Such substructures are provided in the Map named
 * claimsWithDisclosure, which is cascading this class.
 * <p>
 * Discloseable members at this level are provided in the disclosures list. These entries will form
 * the content of the "_sd" Array.
 * </p>
 * <p>
 * The arrayEntryMap holds a map of listed hash values that is to be serialized to JSON to reveal
 * the discloseable members of each value array.
 * </p>
 *
 */
@Getter
public class ClaimsWithDisclosure {
  ...
  /**
   * Claims on this level that has a list of values, where each value is either a value object or a
   * selectable disclosure reference. Eg: <code>
   *   "nationalities": [
   *         "NO",
   *        {
  ...
  /**
   * Any members here are only included in case there is a sublevel of disclosable items. This could
   * be the case, for example, if a claim "Address" has a substructure of individual subclaims each
   * supporting selectable disclosure.
   */
  private Map<String, ClaimsWithDisclosure> claimsWithDisclosure;

  /**
   * Parses a map of claims into an object that maps claims to their corresponding disclosures using
   * a provided list of disclosures and a specific token digest algorithm.
   *
   ...
   * @param sdAlg the token digest algorithm used for generating or validating hash strings of
   *        disclosures.
   * @return an instance of ClaimsWithDisclosure containing processed claims and their associated
   *         disclosures.
  ...
  public static ClaimsWithDisclosure parse(
      ...
      TokenDigestAlgorithm sdAlg) throws NoSuchAlgorithmException {
  ...
  /**
   * Retrieves all disclosures associated with the current object, including disclosures stored in
   * the list and any nested disclosures within claims.
   *
  ...
  /**
   * Get all claims that need to be present in addition to the _sd selectably discolable claims and
   * values
   *
   * @return claims to include in the issuer signed jwt with selectable disclosure
   * @throws NoSuchAlgorithmException if an unsupported algorithm is encountered while computing
   *         hashes.
   */
  ...
  /**
   * Creates a new instance of ClaimsWithDisclosureBuilder using the specified token digest
   * algorithm.
   *
   * @param hashAlgo the token digest algorithm to be used for handling claim disclosures.
   * @return a new instance of ClaimsWithDisclosureBuilder initialized with the provided token
   *         digest algorithm.
   */
  ...
  /**
   * A builder class for constructing an instance of ClaimsWithDisclosure. Provides methods for
   * adding claims, disclosures, and other related data to create a finalized ClaimsWithDisclosure
   * object.
   */
  public static class ClaimsWithDisclosureBuilder {
    ...
    /**
     * Constructs a new ClaimsWithDisclosureBuilder instance. Initializes an empty
     * ClaimsWithDisclosure object and sets up the hashing algorithm for disclosures based on the
     * provided TokenDigestAlgorithm.
     *
    ...
    /**
     * Adds an entry to the specified claim as an array. If the value provided is a
     * {@link Disclosure} object, it will be hashed using the defined hashing algorithm and stored
     * as a reference. Otherwise, the value is directly added to the list for the claim.
     *
     * @param claimName the name of the claim to which the value or disclosure will be added
     * @param valueOrDisclosure the value or {@link Disclosure} object to be added to the claim's
     *        array
     * @return the current instance of {@link ClaimsWithDisclosureBuilder} for method chaining
    ...
    /**
     * Adds an entry to the open claims within the ClaimsWithDisclosure being built. The specified
     * key-value pair is stored in the open claims map, meaning that this claim is presented in
     * clear text and will be available always without any disclosure data.
     *
    ...
    /**
     * Adds a sublevel ClaimsWithDisclosure structure to this ClaimsWithDisclosure object under a
     * claim name as key.
     *
