...
/**
 * Implementation of the PresentationValidator interface for validating SD JWT (Selective Disclosure
 * JSON Web Token) presentations. This class performs validation on given SD JWT presentations,
 * ensuring compliance with expected security and integrity constraints, including checking the
 * wallet key binding and validating the request nonce.
 * <p>
 * This validator supports optional configuration for handling time skew during validation through
 * the timeSkew parameter. If no configuration is provided, a default time skew of 30 seconds is
 * applied.
 * <p>
 * Validation involves verifying the integrity of the SD JWT, ensuring appropriate key binding
 * protection, and checking that the nonce provided by the presentation aligns with the expected
 * request nonce.
 */
@Slf4j
public class SdJwtPresentationValidator implements PresentationValidator {
   ...
   *
   * Initializes the instance with a default time skew value of 30 seconds. This time skew is used
   * during the validation process to account for minor time discrepancies between systems, ensuring
   * robust handling of time-dependent SD JWT validation constraints.
   *
   * The SdKwtPresentationValidator class is used to validate SD JWT presentations, ensuring their
   * integrity, security, and compliance with wallet key binding requirements and the expected
   * request nonce.
   */
  ...
  /**
   * Constructs an SdKwtPresentationValidator with a specified time skew. The time skew is used to
   * handle potential minor discrepancies in system clocks during validation processes.
   *
   * @param timeSkew the allowed time deviation between systems for validation purposes. Must not be
   *        null.
   */
  ...
  /**
   * Validates an SD JWT presentation by ensuring proper key binding and conformity to the expected
   * input values.
   * <p>
   * If No trusted keys are provided, any key will be allowed as signing key. If this option is
   * used, the verifier must validate through other means that the signing key provided in the
   * result used to sign is trusted.
   *
   * @param presentation the byte array representation of the SD JWT presentation to be validated
   * @param presentationValidationInput the validation input containing required parameters such as
   *        request nonce
   * @param trustedKeys an optional list of trusted keys used to verify the presentation's integrity
   * @return A {@code TokenValidationResult} containing the result of the validation process,
   *         including status and additional data.
   * @throws TokenValidationException If the validation fails due to invalid or missing values, or
   *         key binding issues.
   */
  @Override
